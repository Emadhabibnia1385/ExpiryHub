...
def main_menu_kb(is_admin: bool = True):
    if not is_admin:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(start_text(), reply_markup=main_menu_kb(is_admin_user(user.id)))
    return MENU
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ ریست شد.\n\n" + start_text(),
        reply_markup=main_menu_kb(is_admin_user(user.id)),
    )
    return MENU
...
async def go_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    context.user_data.clear()
    await q.edit_message_text(start_text(), reply_markup=main_menu_kb(is_admin_user(q.from_user.id)))
    return MENU
...
async def type_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not title:
        await q.edit_message_text(tr("no_types"), reply_markup=main_menu_kb(is_admin_user(q.from_user.id)))
        return MENU
